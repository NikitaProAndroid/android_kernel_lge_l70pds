...
#define DSV_VEE_EN 111
#define DSV_VDD_EN 56	//for Rev.B...
#endif
#endif
...
static int is_first_dsv_control = 1;
#endif
#endif
	...
	if (enable) {
		...
		if(lge_get_board_revno() >= HW_REV_B)
		{
			if(gpio_is_valid(ctrl_pdata->disp_en_gpio))
			{
				...
				pr_info("DSV_VEE(111) off(lcd_dsv_enp_gpio). \n");
			}
			else
			{
		...
		if(gpio_is_valid(ctrl_pdata->lcd_dsv_enp_gpio))
		{
			...
			pr_info("DSV_VDD(56) off(disp_en_gpio). \n");
		}
		else
		{
			pr_err("%s:%d, disp_en_gpio gpio not specified\n",
							__func__, __LINE__);
		}
		#endif
...
#if defined(CONFIG_MACH_MSM8926_X10_VZW) || defined(CONFIG_MACH_MSM8926_B2L_ATT) || defined(CONFIG_MACH_MSM8926_B2LN_KR) || defined(CONFIG_MACH_MSM8926_JAGN_KR) || defined(CONFIG_MACH_MSM8926_JAGNM_GLOBAL_COM) || defined(CONFIG_MACH_MSM8926_JAGNM_KDDI_JP) || defined(CONFIG_MACH_MSM8926_JAGNM_KDDI_JP) || defined(CONFIG_MACH_MSM8226_JAG3GSS_GLOBAL_COM) || defined(CONFIG_MACH_MSM8226_JAG3GDS_GLOBAL_COM) || defined(CONFIG_MACH_MSM8926_AKA_KR)  || defined(CONFIG_MACH_MSM8926_F60S_JP)
		if(HW_REV_0 == hw_rev)
#endif
    	{
...
#if defined(CONFIG_MACH_MSM8926_E2_MPCS_US) || defined(CONFIG_MACH_MSM8926_E2_SPR_US) || defined(CONFIG_MACH_MSM8926_E2_VTR_CA)
...
#elif defined CONFIG_LGE_MIPI_DSI_LGD_LVDS_WXGA				//E10Wifi
		mdelay(20);
		mdss_dsi_panel_reset(pdata, 0);
		if(lge_lvds_panel_power(pdata, 0)){
...
int mdss_dsi_on(struct mdss_panel_data *pdata)
{
	...
	else
	{
		...
		pr_err("%s : dsv_control is not allowed after this time. dsv_control_enable = [%d]\n", __func__, dsv_control_enable);
		/* set dsv status to be off in order to reconfirm dsv status is off */
		rc = gpio_tlmm_config(GPIO_CFG(ctrl_pdata->disp_en_gpio,
									0,
									GPIO_CFG_OUTPUT,
									GPIO_CFG_PULL_DOWN,
									GPIO_CFG_2MA),
		...
		pr_info("%s: gpio_tlmm_config(%d), rc = %d\n", __func__, ctrl_pdata->disp_en_gpio, rc);
...
#if defined CONFIG_LGE_MIPI_DSI_LGD_LVDS_WXGA		//E10Wifi
{
	u32 tmp;
	if(lge_lvds_panel_power(pdata,1)){
		pr_err("%s:Failed to Enable Panel_Power!\n",__func__);
		return 0;
	}
...
#elif defined CONFIG_LGE_MIPI_DSI_LGD_NT35521_WXGA	//E7Wifi & E7LTE
{
	...
	if(lge_get_board_revno() >= HW_REV_B)
	{
		if(gpio_is_valid(ctrl_pdata->lcd_dsv_enp_gpio))
		{
			gpio_set_value((ctrl_pdata->lcd_dsv_enp_gpio), 1);
			pr_info("DSV_VDD(56) ON(lcd_dsv_enp_gpio). \n");
			udelay(50);
		}
		else
		{
	...
	if(gpio_is_valid(ctrl_pdata->disp_en_gpio))
	{
		...
		pr_info("DSV_VEE(111) ON(disp_en_gpio). \n");
	}
	else
	{
		pr_err("%s:%d, disp_en_gpio gpio not specified\n",
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
...
#if defined(CONFIG_MACH_MSM8926_JAGNM_ATT) || defined(CONFIG_MACH_MSM8926_JAGC_SPR) || defined(CONFIG_MACH_MSM8926_JAGNM_GLOBAL_COM) || defined(CONFIG_MACH_MSM8926_JAGNM_KDDI_JP) || defined(CONFIG_MACH_MSM8926_JAGN_KR) || defined(CONFIG_MACH_MSM8926_JAGNM_RGS) || defined(CONFIG_MACH_MSM8926_JAGNM_VTR) || defined(CONFIG_MACH_MSM8926_AKA_KR)  || defined(CONFIG_MACH_MSM8926_F60S_JP)
	if(lge_get_board_revno() >= HW_REV_B)
		ctrl_pdata->disp_en_gpio = 60;//for just safe rev B SMT
	else
		ctrl_pdata->disp_en_gpio = 111;//for just safe rev B SMT
#endif
#if defined(CONFIG_MACH_MSM8926_JAGDSNM_CN)
	if(lge_get_board_revno() >= HW_REV_C)
		ctrl_pdata->disp_en_gpio = 60;
	else
...
#endif
	{
		...
		if (!gpio_is_valid(ctrl_pdata->disp_p_mode)) {
			...
			rc = gpio_request(ctrl_pdata->disp_p_mode, "disp_p_mode");
			if (rc) {
				pr_err("request disp_p_mode gpio failed, rc=%d\n", rc);
				gpio_free(ctrl_pdata->disp_p_mode);
				return -ENODEV;
			}
		}
	}
	...
	if(lge_get_board_revno() >= HW_REV_B)
	{
		ctrl_pdata->lcd_dsv_enp_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			"qcom,platform-lcd_dsv_enp_gpio", 0);
		pr_info("%s  platform-lcd_dsv_enp_gpio : %d \n", __func__, ctrl_pdata->lcd_dsv_enp_gpio);
		if (!gpio_is_valid(ctrl_pdata->lcd_dsv_enp_gpio)) {
			pr_err("%s:%d, lcd_dsv_enp_gpio gpio not specified\n",
							__func__, __LINE__);
		} else {
			rc = gpio_request(ctrl_pdata->lcd_dsv_enp_gpio, "lcd_dsv_enp_gpio");
			if (rc) {
				pr_err("request reset gpio failed, rc=%d\n",
				       rc);
