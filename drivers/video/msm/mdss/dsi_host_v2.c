...
int first_on=1;
void check_status_func(void);
extern int get_l70p_first_boot(void);
extern void set_l70p_first_boot(bool value);
...
#if defined(CONFIG_MACH_MSM8X10_L70P)
void msm_dsi_bllp_enable(struct mdss_dsi_ctrl_pdata *ctrl, bool enable)
{
	u32 data;
	unsigned char *ctrl_base = dsi_host_private->dsi_base;
	pr_err("[L70P]%s : %s\n",__func__,enable==0?"Disable":"Enable");
	msm_dsi_wait4video_eng_busy(ctrl);
	data = MIPI_INP(ctrl_base + DSI_VIDEO_MODE_CTRL);
	if(enable)
		data |= BIT(12);
	else
		data &= ~BIT(12);
	MIPI_OUTP(ctrl_base + DSI_VIDEO_MODE_CTRL, data);
	pr_err("[L70P]%s : %s complete\n",__func__,enable==0?"Disable":"Enable");
}
#endif
...
void check_status_func(void)
{
        ...
        if(first_on){
            dsi_status=MIPI_INP(ctrl_base +0x013C);
            pr_err("check status : %x\n",dsi_status);
            if(dsi_status == 0xffffffff){
				status++;
				if(status > 5){
	                MIPI_OUTP(ctrl_base + DSI_SOFT_RESET, 0x01);
	                wmb();
	                MIPI_OUTP(ctrl_base + DSI_SOFT_RESET, 0x00);
	                wmb(); 
	                pr_err("reset dsi controller\n");
					status = 0 ; 
					first_on = 0;
				}
			}else{
                pr_err("Clear status : %d\n",status);
				status = 0 ; 
				first_on = 0;
			}
        }
...
static int msm_dsi_on(struct mdss_panel_data *pdata)
{
	...
	msleep(5);
		...
		set_l70p_first_boot(false);
		pr_err("%s : L70P is first boot, doesnt' dsv off this time", __func__);
	}
#endif
#endif
