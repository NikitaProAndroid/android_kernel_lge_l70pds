		...
		u8 drv_type)
{
    ...
    pr_debug("%s: %s: set card drive type to %d\n",
            mmc_hostname(mmc), __func__, drv_type);
}
#endif
...
#if defined(CONFIG_MMC_DATA_CRC_TUNING)
			pr_info("%s: %s: found good phase = %d\n", mmc_hostname(mmc), __func__, phase);
#else
...
#if defined(CONFIG_MMC_DATA_CRC_TUNING)
    pr_debug("%s: %s: PASS phase cnt = %d\n",
				mmc_hostname(mmc), __func__, tuned_phase_cnt);
...
{
    /* bohyun, jung@lge.com - QCT Patch for Data CRC (SR01858183)
