...
static const struct mmc_fixup mmc_fixups[] = {
    ...
    /*
     * Some Micron cards reports error during cache off, and data loss even if FLUSH_CACHE command sent.
     * Disable cache for all versions until a class of cards that
     * show this behavior is identified.
     */
#if defined(CONFIG_LGE_MMC_DISABLE_CACHE_FOR_MICRON)
    MMC_FIXUP(CID_NAME_ANY, CID_MANFID_NUMONYX_MICRON,
                  CID_OEMID_ANY, add_quirk_mmc, MMC_QUIRK_CACHE_DISABLE),
#endif
	END_FIXUP
};
...
#if defined (CONFIG_MMC_DATA_CRC_TUNING)
	...
	struct mmc_card *oldcard)
{
	...
	if (!oldcard) {
         ...
         */
#if defined(CONFIG_LGE_MMC_DISABLE_CACHE_FOR_MICRON)
        if ( card->cid.manfid == CID_MANFID_NUMONYX_MICRON ) {
            add_quirk_mmc(card,MMC_QUIRK_CACHE_DISABLE);
        }
#endif
